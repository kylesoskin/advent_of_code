...
def get_antinodes(p1, p2)
  ...
  horz_diff = col2 - col1
  anti_nodes = []
  until out_of_bounds(row1, col1)
    anti_node1 = [row1 - vert_diff, col1 - horz_diff]
    anti_nodes << anti_node1
    row1, col1 = anti_node1
  end
  until out_of_bounds(row2, col2)
    anti_node2 = [row2 + vert_diff, col2 + horz_diff]
    anti_nodes << anti_node2
    row2, col2 = anti_node2
  end
...
an_count = @input_display.flatten.count { |e| e != '.' }
