...
def diag_left_up_str(row,col) 
  return nil if [[row,col], [row-1,col-1], [row-2,col-2], [row-3,col-3]].any? {|r,c| out_of_bounds(r,c)}
  @input[row][col] +
  @input[row-1][col-1] +
  @input[row-2][col-2] +
  @input[row-3][col-3]
...
(0..@row_index_max).each do |row|
  (0..@col_index_max).each do |col|
    results << {row:, col:, start_letter: @input[row][col], count: search(row,col).count("XMAS"), arr_result: search(row,col)}
  end
end
